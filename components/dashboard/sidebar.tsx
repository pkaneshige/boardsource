...
// Default navigation items - can be overridden with CMS data later
...
function Sidebar({ className = "" }: SidebarProps) {
  ...
  const activeNavStyles = "bg-gray-100 text-gray-900 dark:bg-gray-800 dark:text-white";
  const inactiveNavStyles =
    "text-gray-600 hover:bg-gray-50 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-white";

  return (
    <aside
      className={`flex h-full w-64 flex-col border-r border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-900 ${className}`}
    >
      {/* Logo / Brand */}
      <div className="flex h-16 items-center border-b border-gray-200 px-6 dark:border-gray-700">
        <Link href="/dashboard" className="flex items-center gap-2">
          ...
          <span className="text-lg font-semibold text-gray-900 dark:text-white">BoardSource</span>
        </Link>
      </div>
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {defaultNavItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={`${baseNavStyles} ${isActive(item.href) ? activeNavStyles : inactiveNavStyles}`}
          >
            {item.icon}
            {item.label}
          </Link>
